    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-6">Survei Anda telah berhasil dikirim.</p>
          <button onClick={resetForm} className="w-full py-4 bg-green-500 text-white rounded-2xl font-bold text-lg">
            Isi Survei Lagi
          </button>
        </div>
      </div>
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between max-w-lg mx-auto">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-gray-800">Survei Kepuasan</span>
            </div>
      ...
      {unitName && autoFill && (
        <div className="bg-green-50 px-4 py-2">
          <div className="max-w-lg mx-auto flex items-center gap-2">
            ...
            <span className="text-sm text-green-700 font-medium">{unitName}</span>
          </div>
        </div>
      )}
        ...
        <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
          {currentStep === 1 && (
            <div className="space-y-6 animate-fadeIn">
              <h2 className="text-xl font-bold text-gray-800">Jenis Layanan</h2>
              <div className="grid grid-cols-2 gap-3">
                {[{ value: 'rawat_jalan', label: 'Rawat Jalan', icon: '🏥' },
                  ...
                  <button key={type.value} type="button" onClick={() => handleRadioChange('service_type', type.value)}
                    className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${formData.service_type === type.value ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_anonymous" checked={formData.is_anonymous} onChange={handleInputChange} className="w-5 h-5 rounded" />
          ...
          {currentStep === 2 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setCurrentStep(1)} className="p-2 text-gray-400">
                  <span className="material-symbols-outlined">arrow_back</span>
                </button>
                <h2 className="text-xl font-bold text-gray-800">Data Diri</h2>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-3">Usia</label>
                <div className="grid grid-cols-2 gap-2">
                  {['< 20 Th', '20-40 Th', '41-60 Th', '> 60 Th'].map((range) => (
                    <button key={range} type="button" onClick={() => handleRadioChange('age', range)}
                      className={`p-3 rounded-xl border-2 text-sm font-medium ${formData.age === range ? 'border-green-500 bg-green-500 text-white' : 'border-gray-200'}`}>
                      {range}
                    </button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{ value: 'male', label: 'Laki-laki', icon: '👨' }, { value: 'female', label: 'Perempuan', icon: '👩' }].map((opt) => (
                    <button key={opt.value} type="button" onClick={() => handleRadioChange('gender', opt.value)}
                      className={`p-4 rounded-2xl border-2 flex items-center justify-center gap-2 ${formData.gender === opt.value ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
              ...
              <button type="button" onClick={() => setCurrentStep(3)} className="w-full py-4 bg-green-500 text-white rounded-2xl font-bold text-lg">Lanjutkan</button>
            </div>
          )}
          ...
          {currentStep === 3 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setCurrentStep(2)} className="p-2 text-gray-400">
                  <span className="material-symbols-outlined">arrow_back</span>
                </button>
                <h2 className="text-xl font-bold text-gray-800">Penilaian Layanan</h2>
              </div>
              <div className="flex justify-between px-2 py-3 bg-gray-100 rounded-xl text-xs">
                ...
                {questions.map((q) => (
                  <div key={q.id} className="bg-gray-50 rounded-2xl p-4">
                    <div className="flex items-start gap-3 mb-3">
                      <span className="w-8 h-8 bg-green-100 text-green-600 rounded-lg flex items-center justify-center text-xs font-bold">{q.code}</span>
                      <div>
                    ...
                    <div className="flex gap-2">
                      {ratingOptions.map((opt) => (
                        <button key={opt.value} type="button" onClick={() => handleRadioChange(q.id, opt.value)}
                          className={`flex-1 py-3 rounded-xl border-2 text-lg ${formData[q.id as keyof FormData] === opt.value ? `${opt.color} text-white border-transparent` : 'border-gray-200'}`}>
                          {opt.emoji}
                        </button>
                      ))}
                    </div>
              ...
              <button type="button" onClick={() => setCurrentStep(4)} disabled={!questions.every(q => formData[q.id as keyof FormData])}
                className="w-full py-4 bg-green-500 text-white rounded-2xl font-bold text-lg disabled:opacity-50">Lanjutkan</button>
            </div>
          )}
          ...
          {currentStep === 4 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setCurrentStep(3)} className="p-2 text-gray-400">
                  <span className="material-symbols-outlined">arrow_back</span>
                </button>
                <h2 className="text-xl font-bold text-gray-800">Kepuasan Keseluruhan</h2>
              </div>
              <div className="bg-yellow-50 rounded-2xl p-6 text-center">
                <p className="text-gray-600 mb-4">Berikan rating keseluruhan</p>
                <div className="flex justify-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} type="button" onClick={() => handleRadioChange('overall_satisfaction', star.toString())} className="p-1">
                      <span className={`text-4xl ${parseInt(formData.overall_satisfaction) >= star ? 'text-yellow-500' : 'text-gray-300'}`}>★</span>
                    </button>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Saran & Masukan (Opsional)</label>
                <textarea name="suggestions" value={formData.suggestions} onChange={handleInputChange} rows={4}
                  className="w-full px-4 py-4 rounded-2xl border border-gray-200 text-lg resize-none" placeholder="Berikan saran..." />
              </div>
              <button type="submit" disabled={submitting || !formData.overall_satisfaction}
                className="w-full py-4 bg-green-500 text-white rounded-2xl font-bold text-lg disabled:opacity-50">
