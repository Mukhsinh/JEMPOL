...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { start_date, end_date, unit_id, group_by = 'kabupaten_kota' } = req.query;

    // Validate group_by parameter
    const validGroupBy = ['kabupaten_kota', 'kecamatan', 'kelurahan'];
    const groupByField = validGroupBy.includes(group_by as string) 
      ? group_by as string 
      : 'kabupaten_kota';
    ...
    surveys?.forEach(survey => {
      const address = survey[groupByField];
      if (address && address.trim() !== '') {
        const normalizedAddress = address.trim();
        addressCount.set(
          normalizedAddress,
          (addressCount.get(normalizedAddress) || 0) + 1
        );
        totalCount++;
      }
    });
