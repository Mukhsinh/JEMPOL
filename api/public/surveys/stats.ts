...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Calculate average scores for each question (q1-q11)
    ...
    // Calculate IKM score (average of all 8 questions, converted to 0-100 scale)
    ...
    const ikm_score = validScores.length > 0 
      ? (validScores.reduce((a, b) => a + b, 0) / validScores.length) * 20 // Convert 1-5 scale to 0-100
      : 0;

    // Calculate NPS (simplified - based on overall satisfaction)
