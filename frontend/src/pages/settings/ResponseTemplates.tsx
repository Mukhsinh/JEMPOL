...
const ResponseTemplates: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTemplate) {
        ...
        const response = await api.put(`/response-templates/${editingTemplate.id}`, formData);
        if (response.data.success) {
        ...
        // Create new template
        const response = await api.post('/response-templates', formData);
        if (response.data.success) {
          await fetchTemplates();
          setShowModal(false);
        }
  ...
  const handleDeleteTemplate = async (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus template ini?')) {
      try {
        const response = await api.delete(`/response-templates/${id}`);
        if (response.data.success) {
          await fetchTemplates();
        }
      } catch (error) {
