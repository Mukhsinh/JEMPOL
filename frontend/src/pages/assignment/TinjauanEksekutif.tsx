        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-3 overflow-x-auto pb-2 items-center">
            <button
              ...
              className={`flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full px-4 shadow-sm transition-transform active:scale-95 ${
                activeFilter === 'all' ? 'bg-slate-900 text-white border border-slate-900' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full px-4 shadow-sm transition-colors group ${
                activeFilter === 'escalation' ? 'bg-red-600 text-white border border-red-600' : 'bg-white text-red-600 border border-red-200 hover:bg-red-50 hover:border-red-300'
              }`}
            >
            ...
            <button
              ...
              className={`flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full px-4 shadow-sm transition-colors ${
                activeFilter === 'sla' ? 'bg-orange-600 text-white border border-orange-600' : 'bg-white text-orange-600 border border-orange-200 hover:bg-orange-50 hover:border-orange-300'
              }`}
            >
            ...
            <button className="flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full bg-white border border-slate-200 hover:bg-slate-50 px-4 text-slate-500 transition-colors shadow-sm">
              <span className="text-sm font-medium">IGD & Rawat Jalan</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="flex flex-col gap-2 rounded-xl p-5 bg-white dark:bg-slate-800 border border-red-100 shadow-[0_2px_10px_-3px_rgba(220,38,38,0.1)] relative overflow-hidden group hover:shadow-md transition-all cursor-pointer ring-1 ring-transparent hover:ring-red-200">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <p className="text-red-700 text-xs font-bold leading-normal">+{stats.new_escalations} Baru (Perlu Tindakan)</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 rounded-xl p-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group hover:border-orange-300 transition-all cursor-pointer hover:shadow-md">
            ...
            <div className="mt-2">
              <p className="text-emerald-600 text-xs font-bold leading-normal bg-emerald-50 w-fit px-2 py-1 rounded-md flex items-center gap-1">
                ...
                Turun {stats.sla_improvement}% (Membaik)
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 rounded-xl p-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group hover:border-blue-300 transition-all">
            ...
            <div className="mt-2">
              <p className="text-emerald-600 text-xs font-bold leading-normal bg-emerald-50 w-fit px-2 py-1 rounded-md flex items-center gap-1">
                ...
                {stats.response_improvement} Lebih Cepat
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 rounded-xl p-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden group hover:border-purple-300 transition-all">
            ...
            <div className="mt-2">
              <p className="text-slate-500 text-xs font-bold leading-normal bg-slate-100 w-fit px-2 py-1 rounded-md">Stabil vs Minggu Lalu</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4 rounded-xl bg-white dark:bg-slate-800 p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              ...
              <button className="text-primary text-sm font-bold hover:bg-blue-50 px-3 py-1 rounded-lg transition-colors">Lihat Detail Grafik</button>
            </div>
            <div className="flex items-end justify-between h-64 gap-2 md:gap-4 pt-8 pb-2 border-b border-l border-slate-100 relative mt-2 pl-2">
              <div className="absolute -left-8 top-0 h-full flex flex-col justify-between text-xs text-slate-400 font-medium">
              ...
              {chartData.map((data, idx) => (
                <div key={idx} className="flex flex-col items-center gap-3 flex-1 group z-10 cursor-pointer">
                  <div className="flex items-end gap-1 h-full w-full justify-center relative">
                    ...
                    <div className={`w-3 bg-primary rounded-t-sm shadow-sm relative`} style={{ height: `${data.resolved}%` }}>
                      {idx === 4 && <div className="absolute -top-2 right-0 w-2 h-2 bg-red-500 rounded-full animate-pulse ring-2 ring-white"></div>}
                    </div>
                    <div className="absolute -top-10 bg-slate-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20 pointer-events-none">
                      Masuk: {data.incoming} | Selesai: {data.resolved}
                    </div>
                  </div>
                  <span className={`text-xs font-semibold ${idx === 4 ? 'text-red-500 bg-red-50 px-1 rounded' : 'text-slate-500'}`}>{data.day}</span>
                </div>
              ))}
            </div>
            <div className="flex justify-center gap-6 mt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-500">Tiket Masuk</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-primary rounded-sm shadow-sm"></div>
                <span className="text-xs font-bold text-slate-500">Tiket Selesai</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="h-full flex flex-col rounded-xl overflow-hidden shadow-md relative bg-slate-900 group">
              <div className="absolute inset-0 z-0 bg-cover bg-center opacity-30 mix-blend-luminosity group-hover:scale-105 transition-transform duration-700" style={{ backgroundImage: 'url("https://images.unsplash.com/photo-1639322537228-f710d846310a?w=400")' }}></div>
              ...
              <div className="relative z-10 flex flex-col justify-between h-full p-6">
                ...
                <div className="mt-8">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-white text-lg font-bold leading-tight">{aiInsight.title}</h3>
                  </div>
                  <p className="text-blue-100 text-sm font-medium leading-relaxed mb-6 opacity-90">
                    {aiInsight.description.split(`${aiInsight.percentage}%`)[0]}
                    <span className="text-white font-bold bg-white/20 px-1 rounded">{aiInsight.percentage}%</span>
                    {aiInsight.description.split(`${aiInsight.percentage}%`)[1]}
                  </p>
                  <button className="w-full flex items-center justify-center rounded-lg h-10 px-4 bg-white text-primary text-sm font-bold hover:bg-slate-100 transition-colors shadow-lg">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <span className="bg-red-100 text-red-700 text-xs font-bold px-2 py-0.5 rounded-full border border-red-200">Perlu Review: 3</span>
            </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-800">
            {loading ? (
              ...
              <>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                      {tickets.map((ticket) => (
                        <tr key={ticket.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group ${ticket.status_type === 'escalation_director' ? 'hover:bg-red-50/30' : ''}`}>
                ...
                <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 flex justify-center">
                  <button className="text-slate-500 text-sm font-medium hover:text-primary transition-colors flex items-center gap-1">
                    Tampilkan 5 tiket lainnya
