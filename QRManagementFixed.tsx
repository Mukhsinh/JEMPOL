...
const QRManagement: React.FC = () => {
  ...
  useEffect(() => {
    loadData();
  }, [currentPage, searchTerm, statusFilter, isAuthenticated, user]);
  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        if (unitsError.message?.includes('Token tidak valid') || 
            unitsError.message?.includes('Authentication required') ||
            unitsError.message?.includes('403') ||
            unitsError.message?.includes('401')) {
          console.warn('🔐 Authentication error on units, redirecting...');
          window.location.href = '/login';
          return;
        }
        
        // Continue with empty units array for other errors
        setUnits([]);
      }
      ...
      try {
        ...
        if (qrError.message?.includes('Token tidak valid') || 
            ...
            qrError.message?.includes('403') ||
            qrError.message?.includes('401')) {
          console.warn('🔐 Authentication error on QR codes, redirecting...');
          window.location.href = '/login';
          return;
        }
      ...
      // Check for auth errors in general catch
...
etStatusBadge = (isActive: boolean) => {
    if (isActive) {
      return (
  ...
  // Show authentication message if still loading or not authenticated
  if (!isAuthenticated || !user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-light dark:bg-background-dark">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-12">
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-slate-500 dark:text-slate-400">Memuat data QR Code...</p>
            </div>
          </div>
        ) : qrCodes.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-6">
              Mulai dengan membuat QR Code pertama untuk unit Anda.
            </p>
            <button
              onClick={() => setShowModal(true)}
