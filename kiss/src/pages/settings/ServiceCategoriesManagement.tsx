...
const ServiceCategoriesManagement: React.FC = () => {
  ...
  const loadCategories = async () => {
    try {
      ...
      setError('Gagal memuat data kategori layanan');
  ...
  const handleExport = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      + "Nama,Kode,Deskripsi,SLA Default,Lampiran Wajib,Status\n"
      + categories.map(item => 
          `"${item.name}","${item.code}","${item.description || ''}","${item.default_sla_hours} jam","${item.requires_attachment ? 'Ya' : 'Tidak'}","${item.is_active ? 'Aktif' : 'Tidak Aktif'}"`
        ).join("\n");
    
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "kategori_layanan.csv");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Kategori Layanan</h2>
          <p className="text-slate-500 mt-1">Kelola kategori layanan dan klasifikasi untuk tiket.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="inline-flex items-center justify-center gap-2 bg-primary hover:bg-blue-600 text-white font-medium px-4 py-2 rounded-lg transition-colors text-sm shadow-sm shadow-blue-500/20">
      ...
      <div className="bg-surface-light dark:bg-surface-dark border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-500 uppercase bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold" scope="col">Lampiran Wajib</th>
                ...
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                  <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                        <span className="material-symbols-outlined text-blue-600 dark:text-blue-400 text-[16px]">category</span>
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${
                      ...
                      <span className="material-symbols-outlined text-[12px]">
                        {item.requires_attachment ? 'attach_file' : 'remove'}
                      </span>
                      {item.requires_attachment ? 'Wajib' : 'Opsional'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-1.5 text-slate-400 hover:text-primary hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors">
        ...
        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <span className="text-sm text-slate-500">Menampilkan {filteredCategories.length} dari {categories.length} entri</span>
