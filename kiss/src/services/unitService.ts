...
class UnitService {
  ...
  async getUnits(params?: { search?: string; type?: string; status?: string }): Promise<{ units: Unit[] }> {
    try {
      console.log('🔄 Fetching units from Supabase with params:', params);
  ...
  async createUnit(unit: Partial<Unit>): Promise<Unit> {
    try {
      ...
      const errorMessage = error.message || 'Gagal membuat unit kerja';
  ...
  async updateUnit(id: string, unit: Partial<Unit>): Promise<Unit> {
    try {
      ...
      const errorMessage = error.message || 'Gagal memperbarui unit kerja';
      throw new Error(errorMessage);
    }
  }
  ...
  async getUnitTypes(): Promise<UnitType[]> {
    // Gunakan Supabase langsung untuk performa lebih baik
    try {
      const { data, error } = await supabase
        .from('unit_types')
        .select('*')
