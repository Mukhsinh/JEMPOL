...
router.post('/tickets/:id/escalate', authenticateSupabase, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.log('📤 Eskalasi tiket:', { id, to_unit_id, reason, user: req.user });
    ...
    if (unitError || !toUnit) {
      console.error('❌ Unit tidak ditemukan:', unitError);
      return res.status(404).json({
        success: false,
    ...
    if (escalationError) {
      ...
      return res.status(500).json({
        success: false,
        error: 'Gagal membuat eskalasi: ' + escalationError.message
      });
    }

    console.log('✅ Eskalasi berhasil dibuat:', escalation.id);
...
router.post('/tickets/:id/respond', authenticateSupabase, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (responseError) {
      ...
      return res.status(500).json({
        success: false,
        error: 'Gagal menambahkan respon: ' + responseError.message
      });
    }

    console.log('✅ Respon berhasil dibuat:', response.id);
